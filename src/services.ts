...
export async function getWeather(location:Location):Promise<Weather> {
 ...
 const url=`https://api.open-meteo.com/v1/forecast?latitude=${location.lat}&longitude=${location.lng}&current=${fields}&hourly=${fields}&forecast_days=3&timezone=auto&wind_speed_unit=kmh`;
 const r=await fetch(url); if(!r.ok) throw new Error('Weather service unavailable'); const data=await r.json(); const c=data.current;
 ...
 const hourly:import('./types').WeatherHour[]=data.hourly.time.map((time:string,i:number)=>{const wind=data.hourly.wind_speed_10m[i]??0,gusts=data.hourly.wind_gusts_10m[i]??0,rain=data.hourly.precipitation[i]??0,rainProbability=data.hourly.precipitation_probability[i]??0,cloud=data.hourly.cloud_cover[i]??0,visibility=data.hourly.visibility[i]??10000,temperature=data.hourly.temperature_2m[i]??0;return{time,temperature:Math.round(temperature),wind:Math.round(wind),gusts:Math.round(gusts),rain,rainProbability,cloud,visibility:Math.round(visibility),score:scoreFor(wind,gusts,rain,rainProbability,cloud,visibility,temperature),isDay:Boolean(data.hourly.is_day[i])}}).filter((hour:import('./types').WeatherHour)=>new Date(hour.time).getTime()>=Date.now()-3600000).slice(0,36);
