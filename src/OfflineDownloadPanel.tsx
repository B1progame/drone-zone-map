...
const MAX_OFFLINE_RADIUS_KM=200;
...
export function OfflineDownloadPanel({location,weather,zoneInfo,onClose,onSaved}:{location:Location;weather?:Weather;zoneInfo?:ZoneInfo;onClose:()=>void;onSaved:()=>void}){
 ...
 return <div className="modalShade offlineShade" role="dialog" aria-modal="true" aria-labelledby="offline-title">
  <section className="settings offlineBuilder liquid">
   ...
   {(scope==='radius'||scope==='city')&&<label className="offlineRadius"><span>Coverage radius <b>{radius.toLocaleString()} km</b></span><input type="range" min={1} max={MAX_OFFLINE_RADIUS_KM} step={1} value={Math.min(MAX_OFFLINE_RADIUS_KM,radius)} onChange={event=>setRadius(Number(event.target.value))}/><input aria-label="Offline coverage radius in kilometres" type="number" min={1} max={MAX_OFFLINE_RADIUS_KM} step={1} value={Math.min(MAX_OFFLINE_RADIUS_KM,radius)} disabled={Boolean(progress)} onChange={event=>setRadius(Math.max(1,Math.min(MAX_OFFLINE_RADIUS_KM,Number(event.target.value)||1)))}/><small>Up to 200 km. At the 200 km maximum, Maximum quality uses native Street zoom 14 and Satellite zoom 13. Large selections are divided automatically.</small></label>}
   ...
   <label className="offlineQuality"><span>Map quality <b>{qualityLabel} · Street z{streetZoom} / Satellite z{satelliteZoom}</b></span><input aria-label="Offline map quality" type="range" min={qualityMin} max={qualityMax} value={basemapTypes.includes('street')?streetZoom:satelliteZoom} disabled={Boolean(progress)} onChange={event=>setQualityOffset(Number(event.target.value)-baseConfig.basemapMaxZoom)}/><small><i>Smaller download</i><i>Maximum native detail: Street z14 · Satellite z13</i></small></label>
