...
export function OfflineDownloadPanel({location,weather,zoneInfo,onClose,onSaved}:{location:Location;weather?:Weather;zoneInfo?:ZoneInfo;onClose:()=>void;onSaved:()=>void}){
 ...
 return <div className="modalShade offlineShade" role="dialog" aria-modal="true" aria-labelledby="offline-title">
  <section className="settings offlineBuilder liquid">
   ...
   <label className="offlineQuality"><span>Map quality <b>{qualityLabel} · zoom {qualityZoom}</b></span><input aria-label="Offline map quality" type="range" min={qualityMin} max={qualityMax} value={qualityZoom} disabled={Boolean(progress)} onChange={event=>setQualityOffset(Number(event.target.value)-baseConfig.basemapMaxZoom)}/><small><i>Smaller download</i><i>More detail</i></small></label>
   <div className="offlineCenter">{basemapType==='satellite'?<Satellite/>:<Map/>}<span><b>Offline {basemapType} map included</b>{basemapType==='satellite'?'Sentinel-2 cloudless imagery':'Vector roads, land and water'} through zoom {qualityZoom} · {shownEstimate.tileCount?.toLocaleString()??'…'} tiles</span></div>
