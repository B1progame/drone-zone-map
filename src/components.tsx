...
export function Nav({page,setPage}:{page:Page;setPage:(p:Page)=>void}) {
  ...
  return <nav ref={dockRef} className="dock" aria-label="Main navigation"><i ref={sliderRef} className="dockSlider" aria-hidden="true"/>{pageMeta.map((x,index)=>{const I=x.icon;return <button ref={node=>{buttonRefs.current[index]=node}} className={page===x.id?'active':''} onClick={()=>setPage(x.id)} aria-label={x.label} aria-current={page===x.id?'page':undefined} key={x.id}><span className="navIcon"><I size={19}/></span><span className="navLabel">{x.label}</span></button>})}</nav>
}
export function SearchBox({onLocation,hero=false}:{onLocation:(l:Location)=>void;hero?:boolean}) {const [value,setValue]=useState(''),[error,setError]=useState(''),[busy,setBusy]=useState(false);const submit=async()=>{setBusy(true);setError('');try{const location=await searchLocation(value);if(location)onLocation(location);else setError('No matching place found. Try a town or coordinates.')}catch{setError('Place search is temporarily unavailable.')}finally{setBusy(false)}};const locate=()=>navigator.geolocation?.getCurrentPosition(position=>onLocation({lat:position.coords.latitude,lng:position.coords.longitude,name:'My location'}),()=>setError('Location permission was not granted.'),{enableHighAccuracy:true,timeout:10000});return <div className={'searchWrap '+(hero?'heroSearch':'')}><Search size={19}/><input value={value} onChange={e=>setValue(e.target.value)} onKeyDown={e=>e.key==='Enter'&&submit()} placeholder="Search Mannheim or paste coordinates" aria-label="Place or coordinates"/><button onClick={submit} disabled={busy}>{busy?'Finding…':<>Check <span>location</span></>}</button><button className="searchLocate" onClick={locate} aria-label="Use my current location" title="Find my location"><LocateFixed/></button>{error&&<small>{error}</small>}</div>}
export function ResultCard({location,weather,zoneInfo,onSave}:{location:Location;weather?:Weather;zoneInfo?:ZoneInfo;onSave:()=>void}) {const loaded=zoneInfo?.status==='loaded';return <aside className="resultCard liquid"><div className="sheetHandle"/><div className="eyebrow">REAL OFFICIAL CLICK RESULT</div><h2>{location.name}</h2><div className={'status '+(loaded?'caution':'unknown')}><span/>{!zoneInfo?'Checking official service…':loaded?`${zoneInfo.zones.length} overlapping ${zoneInfo.zones.length===1?'zone':'zones'} · ${zoneInfo.sourceName}`:zoneInfo.status==='none'?`No zone returned by ${zoneInfo.sourceName}`:zoneInfo.status==='unsupported'?`${zoneInfo.countryName} · no reusable point query`:`${zoneInfo.countryName} · official source unavailable`}</div>{zoneInfo?.zones.length?<div className="zoneResults">{zoneInfo.zones.slice(0,8).map(zone=><details key={zone.id} open={zoneInfo.zones.length===1}><summary><div><b>{zone.name}</b><span>{zone.type}</span></div><i>+</i></summary><div className="zoneBody">{zone.message&&<p>{zone.message}</p>}<dl>{zone.lower&&<><dt>Lower limit</dt><dd>{zone.lower}</dd></>}{zone.upper&&<><dt>Upper limit</dt><dd>{zone.upper}</dd></>}{zone.legalReference&&<><dt>Legal reference</dt><dd>{zone.legalReference}</dd></>}{zone.contact&&<><dt>Contact</dt><dd>{zone.contact}</dd></>}</dl></div></details>)}</div>:zoneInfo&&<p>{zoneInfo.status==='none'?'The official point query returned no overlapping zone. Normal rules and unrepresented restrictions still apply.':zoneInfo.warning}</p>}{weather&&<div className="weatherline"><CloudSun size={19}/><div><b>{weather.score}/100 · {quality(weather.score)}</b><span>{weather.wind} km/h wind · {weather.rainProbability}% rain</span></div></div>}<div className="cardActions"><button onClick={onSave}><Heart size={16}/> Save location</button>{zoneInfo?.sourceUrl&&zoneInfo.sourceUrl!=='#'&&<a href={zoneInfo.sourceUrl} target="_blank" rel="noreferrer">Official source ↗</a>}</div><Disclaimer/></aside>}
