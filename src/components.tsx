...
export function ResultCard({location,weather,zoneInfo,onSave,onClose,language='en'}:{location:Location;weather?:Weather;zoneInfo?:ZoneInfo;onSave:()=>void;onClose:()=>void;language?:string}) {
 ...
 return <aside className="resultCard liquid">
  ...
  <div className={'status '+(loaded?'caution':'unknown')}><span/>{!zoneInfo?t(language,'checking'):loaded?`This point intersects ${zoneInfo.zones.length} official ${zoneInfo.zones.length===1?'zone':'zones'} · ${zoneInfo.sourceName}`:zoneInfo.status==='none'?`No restriction was found in the downloaded or queried official dataset · ${zoneInfo.sourceName}`:zoneInfo.status==='unsupported'?`${zoneInfo.countryName} · official map handoff`:`${zoneInfo.countryName} · official source unavailable`}</div>
  {zoneInfo?.zones.length?<div className="zoneResults">{zoneInfo.zones.slice(0,12).map(zone=><details key={zone.id} open={zoneInfo.zones.length===1}><summary><div><b>{zone.name}</b><span>{zone.type}</span></div><i>+</i></summary><div className="zoneBody">{zone.message&&<p>{zone.message}</p>}{zone.pilotAction&&<p><b>Pilot action:</b> {zone.pilotAction}</p>}<dl>{zone.originalName&&zone.originalName!==zone.name&&<><dt>Original official name</dt><dd>{zone.originalName}</dd></>}{zone.officialLayerName&&<><dt>Official layer</dt><dd>{zone.officialLayerName}</dd></>}{zone.lower&&<><dt>Lower limit</dt><dd>{zone.lower}</dd></>}{zone.upper&&<><dt>Upper limit</dt><dd>{zone.upper}</dd></>}{zone.legalReference&&<><dt>Legal reference</dt><dd>{zone.legalReference}</dd></>}{zone.authority&&<><dt>Authority</dt><dd>{zone.authority}</dd></>}{zone.contact&&<><dt>Contact</dt><dd>{zone.contact}</dd></>}{zone.updated&&<><dt>Source updated</dt><dd>{zone.updated}</dd></>}{zone.sourceUrl&&<><dt>Source</dt><dd><a href={zone.sourceUrl} target="_blank" rel="noreferrer">{zone.source} ↗</a></dd></>}</dl></div></details>)}</div>:zoneInfo&&<p>{zoneInfo.status==='none'?'No restriction was found in the downloaded or queried official datasets. Temporary restrictions and current NOTAMs may not be included. Verify current conditions on the official map before flying.':zoneInfo.warning}</p>}
  {zoneInfo?.zones.length?<p className="zoneCaveat">Temporary restrictions and current NOTAMs may not be included. Verify current conditions on the official map before flying.</p>:null}
  ...
  <div className="cardActions"><button onClick={onSave}><Heart size={16}/> Save location</button>{zoneInfo?.sourceUrl&&zoneInfo.sourceUrl!=='#'&&<a href={zoneInfo.sourceUrl} target="_blank" rel="noreferrer">{t(language,'official')} ↗</a>}</div><Disclaimer/>
 </aside>;
}
