...
import { Bot, CloudSun, Heart, Home, Layers3, LocateFixed, LogOut, Map, Search, Settings, ShieldAlert, Sparkles, X } from 'lucide-react';
...
export function Header({setSettings,account,onLogout}:{setSettings:()=>void;account:LocalAccount;onLogout:()=>void}) {return <header><Logo/><div className="accountControls"><div className="accountBadge liquid"><i>{account.displayName.slice(0,1).toUpperCase()}</i><span><b>{account.displayName}</b><small>{account.role}</small></span></div><button className="icon liquid" onClick={setSettings} aria-label="Settings"><Settings size={19}/></button><button className="icon liquid" onClick={onLogout} aria-label="Sign out" title="Sign out"><LogOut size={18}/></button></div></header>}
