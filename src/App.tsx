...
import { deleteAllOfflinePacks, deleteOfflinePack, downloadGeoJson, formatBytes, getOfflineContext, getOfflinePacks, isOfflinePackStale, refreshOfflinePack, type OfflineDownloadProgress, type OfflinePack } from './offline';
...
function SavedPage({saved,routes,choose,openRoute,remove,removeRoute,update}:{saved:SavedPlace[];routes:SavedRoute[];choose:(l:Location)=>void;openRoute:(route:SavedRoute)=>void;remove:(id:string)=>void;removeRoute:(id:string)=>void;update:(id:string,changes:Partial<SavedPlace>)=>void}){
 ...
 {tab==='storage'&&<section className="offlineLibrary">{offline.length?offline.map(pack=><article className="offlineCard liquid" key={pack.id}><WifiOff/><div><small>OFFLINE PACKAGE · {pack.metadata.country.toUpperCase()} · {new Date(pack.metadata.updatedAt).toLocaleDateString()}</small><h3>{pack.name}</h3><p>{formatBytes(pack.metadata.sizeBytes)} · {pack.metadata.itemCount.toLocaleString()} items · street map{pack.metadata.layers.length?` + ${pack.metadata.layers.map(layer=>layer==='basic'?'infrastructure':layer).join(', ')}`:''}</p><span>{pack.config.scope}{pack.metadata.radiusKm?` · ${pack.metadata.radiusKm} km radius`:''} · v{pack.metadata.version}{isOfflinePackStale(pack)?' · Update available':' · Current'}</span>{offlineAction===pack.id&&offlineProgress&&<div className="offlineInlineProgress"><i><b style={{width:`${offlineProgress.percent}%`}}/></i><small>{offlineProgress.stage} · {offlineProgress.items.toLocaleString()} items</small></div>}<em>{pack.notice}</em></div><div className="offlineActions"><button onClick={()=>choose(pack.location)}>Open <ArrowRight/></button><button onClick={()=>void updatePack(pack)} disabled={!navigator.onLine||offlineAction===pack.id}><RefreshCw/> {offlineAction===pack.id?'Updating…':'Update'}</button><button onClick={()=>downloadGeoJson(pack)}><Download/> GeoJSON</button><button className="delete" onClick={()=>void removePack(pack.id)}><Trash2/> Delete</button></div></article>):<div className="storagePanel liquid"><WifiOff/><div><h2>No offline packages yet</h2><p>Choose a point on the map and use “Download offline area”. Germany, France and Ireland support country, city and custom-radius packages; Germany also supports federal states.</p></div></div>}{offline.length>0&&<button className="danger" onClick={()=>void deleteAllOfflinePacks().then(loadOffline)}>Delete all offline packages</button>}</section>}
 <Disclaimer/></main>
}
