...
import { ArrowRight, Bot, Camera, Check, CircleDot, Cloud, CloudRain, CloudSun, Compass, Database, Download, Eye, Heart, Layers3, Map as MapIcon, Navigation, Pencil, Route, Send, ShieldCheck, Sparkles, Star, Trash2, WifiOff, Wind, X } from 'lucide-react';
...
export default function App(){
 ...
 const choose=(l:Location)=>{const request=++chooseRequest.current;setLocation(l);setPage('map');setWeather(undefined);setZoneInfo(undefined);setWeatherError('');void getWeather(l).then(result=>{if(request===chooseRequest.current)setWeather(result)}).catch(()=>{if(request===chooseRequest.current)setWeatherError('Live weather is temporarily unavailable.')});void getOfficialZoneInfo(l,appSettings.language).then(result=>{if(request===chooseRequest.current)setZoneInfo(result)})};
...
function MapPage({location,weather,weatherError,zoneInfo,choose,save,saveRoute,settings}:{location?:Location;weather?:Weather;weatherError:string;zoneInfo?:ZoneInfo;choose:(l:Location)=>void;save:()=>void;saveRoute:(route:SavedRoute)=>void;settings:AppSettings}){
 const[offline,setOffline]=useState(false),[resultOpen,setResultOpen]=useState(true),[plannerMode,setPlannerMode]=useState(false),[exportStatus,setExportStatus]=useState(''),[directoryOpen,setDirectoryOpen]=useState(false);
 ...
 return <main className="mapPage">
  ...
  <div className={`mapPanel airspaceDirectory liquid${directoryOpen?' compactOpen':''}`}>
   ...
   <div className="directoryActions">
    <button onClick={makePack} disabled={!location}>{offline?<><Check/>Saved offline</>:<><Download/>Save offline</>}</button>
...
function AiPage({location,weather,zoneInfo,saved}:{location?:Location;weather?:Weather;zoneInfo?:ZoneInfo;saved:number}){
 ...
 return <main className="page aiPage"><div className="aiHeader"><div><div className="eyebrow">AERIS COPILOT · GROUNDED FLIGHT CONTEXT</div><h1>Turn map data into clarity.</h1><p>Ask about the loaded location, weather and official-zone context. No login is required; use the built-in assistant or connect an optional OpenRouter key.</p></div><div className="aiPulse"><Bot/><i/></div></div><section className="copilotGrid"><div className="chatPanel liquid"><div className="chatTop"><div className="aiAvatar"><Sparkles/></div><div><b>Aeris Copilot</b><span><i/> {apiKey.trim()?'OpenRouter enabled':'Ready on this device'}</span></div></div><div className="chatBody">{messages.length===0?<div className="assistantMessage"><b>Ready when you are.</b><p>{location?`I have grounded context for ${location.name}${weather?` and its ${weather.score}/100 weather score`:''}.`:'Select a location on the map, then ask a question.'}</p></div>:messages.map((message,index)=><div key={index} className="conversation"><div className="userMessage">{message.question}</div><div className={`assistantMessage${message.error?' error':''}`}><b>{message.error?'Could not answer':'Aeris Copilot'}</b><MarkdownAnswer text={message.error??message.answer??'Thinking…'}/>{message.sourceUrl&&<a className="answerSource" href={message.sourceUrl} target="_blank" rel="noreferrer noopener"><ShieldCheck/> Open {message.sourceName??'official aviation source'} <ArrowRight/></a>}{message.provider&&<small>{message.provider}</small>}</div></div>)}</div><div className="chatInput"><input value={question} disabled={busy} onChange={event=>setQuestion(event.target.value)} onKeyDown={event=>event.key==='Enter'&&ask()} placeholder={busy?'Aeris is checking the context…':'Ask about this flight context…'}/><button disabled={busy} onClick={()=>ask()} aria-label="Send"><Send/></button></div></div><aside className="contextRail"><div className={`openRouterCard liquid ${keyStatus}`}><small>OPENROUTER · OPTIONAL</small><b>Use a free AI model</b><p>The key stays only in this browser tab and is never added to the project or sent anywhere except OpenRouter.</p><div className="keyRow"><input type="password" autoComplete="off" value={apiKey} onChange={event=>{setApiKey(event.target.value);setKeyStatus('idle');setKeyMessage('')}} placeholder="sk-or-v1-…"/><button onClick={testKey} disabled={keyStatus==='testing'}>{keyStatus==='testing'?'Testing…':'Test key'}</button></div>{keyMessage&&<span className="keyStatus">{keyMessage}</span>}<small className="modelName">{OPENROUTER_MODEL}</small>{apiKey&&<button className="disconnectKey" onClick={disconnect}>Remove from this tab</button>}</div><div className="localAssistantCard liquid"><small>ON-DEVICE FALLBACK · NO KEY</small><b>Always available</b><p>Leave the key field empty to answer from weather and airspace data already loaded in the app. No login or provider setup is required.</p></div><div className="contextCard liquid"><small>ACTIVE CONTEXT</small><b>{location?.name??'No location selected'}</b><span>{weather?`${weather.score}/100 weather · ${weather.wind} km/h wind`:'Weather waiting'}</span><span>{zoneInfo?`${zoneInfo.zones.length} zones · ${zoneInfo.sourceName}`:'Official source waiting'}</span><span>{saved} saved {saved===1?'place':'places'}</span></div><div className="quickActions"><small>QUICK QUESTIONS</small>{['Can I fly here?','Explain the visible zones','What is the weather risk?','Best time in 24 hours?'].map(text=><button key={text} onClick={()=>ask(text)}>{text}<ArrowRight/></button>)}</div><div className="aiSafety"><ShieldCheck/><p>No legal permission claims. Always verify the responsible official aviation source.</p></div></aside></section></main>
}
...
function SavedPage({saved,routes,choose,openRoute,remove,removeRoute,update}:{saved:SavedPlace[];routes:SavedRoute[];choose:(l:Location)=>void;openRoute:(route:SavedRoute)=>void;remove:(id:string)=>void;removeRoute:(id:string)=>void;update:(id:string,changes:Partial<SavedPlace>)=>void}){
 const [tab,setTab]=useState<'places'|'routes'|'sources'|'storage'>('places'),[editing,setEditing]=useState(''),offline=getOfflinePacks();
